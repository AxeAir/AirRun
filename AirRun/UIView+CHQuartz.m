...
//  Copyright (c) 2014年 陈 浩. All rights reserved.
//
...
@implementation UIView (CHQuartz)
...
//空心圆
-(void)drawCircleWithNoCenter:(CGPoint)center radius:(float)radius
{
    CGContextRef     context = UIGraphicsGetCurrentContext();
    ...
    CGContextSetLineWidth(context, 2);//设置粗细
                         ...
                         blue:(NSInteger)blue
{
    ...
    CGPathAddArc(pathRef,
                 ...
                 radius*2*PI-PI/2,
                 NO);
    //CGContextSetStrokeColor(context,(float)red/255.0, (float)green/255.0, blue/255.0, 1);
...
-(void)drawLineFrom:(CGPoint)startPoint to:(CGPoint)endPoint color:(UIColor*)color width:(CGFloat)width
{
    CGContextRef     context = UIGraphicsGetCurrentContext();
